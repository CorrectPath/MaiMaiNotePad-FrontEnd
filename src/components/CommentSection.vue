            ...
            <div class="comment-body">
              ...
              >
                <el-input
                  ...
                  :maxlength="500"
                  show-word-limit
                  :placeholder="muteInfo ? '当前处于禁言状态，无法发表评论' : '回复内容（最多500字）'"
...
const rebuildTree = (list) => {
  ...
  list.forEach((raw) => {
    const item = {
      ...raw,
      isOwner: isOwnerUser(raw.userId),
      isAdmin: admin && raw.userId === userId
    }
    if (!item.parentId) {
      items.push(item)
    } else {
...
const handleReact = async (item, action) => {
  ...
  try {
    const response = await reactComment(item.id, nextAction)
    if (response && response.data) {
      item.likeCount = response.data.likeCount
      item.dislikeCount = response.data.dislikeCount
